    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div
          ...
          className="bg-[#fffaf3] p-8 rounded-2xl shadow-md border-2 border-[#5c3a21] mb-10 text-center"
        >
          <img
            src={user.user_metadata?.avatar_url || "/default-avatar.png"}
            className="w-24 h-24 rounded-full mx-auto mb-4"
          />
          <h2 className="text-xl font-bold mb-2">{user.user_metadata?.full_name || "Anonymous"}</h2>
          <p className="mb-6">{user.email}</p>

          <div className="grid md:grid-cols-3 gap-4">
            <div className="p-3 rounded-lg border-2 border-[#5c3a21] font-semibold bg-white">
              <span className="block font-bold text-sm text-[#5c3a21]">User ID</span>
              {user.id}
            </div>
            <div className="p-3 rounded-lg border-2 border-[#5c3a21] font-semibold bg-white">
              <span className="block font-bold text-sm text-[#5c3a21]">Account Created</span>
              {new Date(user.created_at).toLocaleDateString()}
            </div>
            <div className="p-3 rounded-lg border-2 border-[#5c3a21] font-semibold bg-white">
              <span className="block font-bold text-sm text-[#5c3a21]">Last 7 Days</span>
